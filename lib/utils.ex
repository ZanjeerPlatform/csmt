...
defmodule CSMT.Utils do
  @moduledoc """
  Utility functions required by `CSMT.Merkle`, `CSMT.Log`, `CSMT.Map`.
  """

  @type tree :: CSMT.Types.tree()

  @type tree_node :: CSMT.Types.tree_node()

  @type backend :: CSMT.Types.backend()

  @type hash_algorithm :: CSMT.Types.hash_algorithm()

  @type hash :: CSMT.Types.hash()

  @doc """
  Generates a unique TreeId for every tree.
  This TreeId is used by `Storage` and `CSMT.TreeStorage` for persisting `t:tree/0` and `t:tree_node/0`
  """
  ...
  This is required as `CSMT.Trees` stores a map as a `List` of `Tuple`.
  """
  ...
  Called by `CSMT.Merkle.new/4` when a new `CSMT.Map` or `CSMT.Log` has to be created.
  """
  @spec make_tree(binary, hash_algorithm, backend) :: tree
  def make_tree(tree_name, hash_function, backend) do
    CSMT.Trees.Tree.new(
  ...
  def make_node(tree, left, right) do
    ...
    CSMT.Utils.make_node(
      tree,
      CSMT.Utils.make_hash(tree, left.hash <> right.hash),
  ...
  def make_map_node(
        tree,
        left = %CSMT.Nodes.Node{treeId: _, hash: _, children: _, metadata: _, key: _, size: _},
        right
      ) do
    CSMT.Nodes.Node.new(
      treeId: tree.treeId,
      hash: CSMT.Utils.make_hash(tree, left.hash <> right.hash),
  ...
  def make_map_node(tree, key, value) do
    ...
    CSMT.Nodes.Node.new(
      treeId: tree.treeId,
      hash: CSMT.Utils.make_hash(tree, CSMT.Utils.salt_node(key, value)),
      children: [],
      size: 1,
      metadata: value,
      key: key
  ...
  def get_backend_module(backend) do
    backend = CSMT.Utils.tuple_list_to_map(backend)

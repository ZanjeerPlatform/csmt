...
defmodule CSMT do
  ...
  @spec new(binary, CSMT.Types.hash_algorithm(), CSMT.Types.backend()) :: CSMT.Types.tree()
  def new(tree_name, hash_function, backend) do
    tree = CSMT.Utils.make_tree(tree_name, hash_function, backend)
    tree = CSMT.Utils.get_backend_module(backend).init_backend(tree)
    # put an empty leaf to make hash not nil (proto def says it's required), make the size zero
    Map.put(tree, :root, CSMT.Utils.make_hash(tree, <<>>)) |> Map.put(:size, 0)
  end

  # insertion in empty tree
  @spec insert(CSMT.Types.tree(), binary, binary) :: CSMT.Types.tree()
  def insert(tree = %CSMT.Trees.Tree{size: 0}, k, v) do
    root = CSMT.Utils.make_map_node(tree, k, v)
    CSMT.Utils.set_node(tree, root.hash, root)
    Map.put(tree, :root, root.hash) |> Map.put(:size, 1)
  end

  # insertion in non empty tree
  @spec insert(CSMT.Types.tree(), binary, binary) :: CSMT.Types.tree()
  def insert(tree = %CSMT.Trees.Tree{root: root, size: size}, k, v) do
    root = CSMT.Utils.get_node(tree, root)
    ...
    if tree.size > 1 do
      ...
      CSMT.Utils.delete_node(tree, root.hash)
    end

    Map.put(tree, :root, new_root.hash) |> Map.put(:size, size + 1)
  end

  defp do_insert(tree, root = %CSMT.Nodes.Node{children: [left, right]}, k, v) do
    left = CSMT.Utils.get_node(tree, left)
    right = CSMT.Utils.get_node(tree, right)
    ...
    cond do
      l_dist == r_dist ->
        ...
        new_leaf = CSMT.Utils.make_map_node(tree, k, v)
        CSMT.Utils.set_node(tree, new_leaf.hash, new_leaf)
        ...
        if k < min_key do
          ...
          new_root = CSMT.Utils.make_map_node(tree, new_leaf, root)
          CSMT.Utils.set_node(tree, new_root.hash, new_root)
          new_root
        else
          # deletes the existing root from the storage as there would be a new root
          CSMT.Utils.delete_node(tree, root.hash)
          # make new leaf as right child at the new level
          new_root = CSMT.Utils.make_map_node(tree, root, new_leaf)
          CSMT.Utils.set_node(tree, new_root.hash, new_root)
          new_root
        end
      ...
      l_dist < r_dist ->
        ...
        # deletes the existing root from the storage as there would be a new root
        CSMT.Utils.delete_node(tree, root.hash)
        new_root = CSMT.Utils.make_map_node(tree, left, right)
        CSMT.Utils.set_node(tree, new_root.hash, new_root)
        new_root
  ...
  defp do_insert(tree, leaf = %CSMT.Nodes.Node{children: [], metadata: _, key: key}, k, v) do
    new_leaf = CSMT.Utils.make_map_node(tree, k, v)
    CSMT.Utils.set_node(tree, new_leaf.hash, new_leaf)
    ...
    cond do
      ...
      k > key ->
        ...
        new_root = CSMT.Utils.make_map_node(tree, leaf, new_leaf)
        CSMT.Utils.set_node(tree, new_root.hash, new_root)
        new_root

      k < key ->
        # new key will be left child
        new_root = CSMT.Utils.make_map_node(tree, new_leaf, leaf)
        CSMT.Utils.set_node(tree, new_root.hash, new_root)
        new_root
    end
  end

  @spec get_with_inclusion_proof(CSMT.Types.tree(), binary) :: CSMT.Types.audit_proof()
  def get_with_inclusion_proof(tree = %CSMT.Trees.Tree{root: root}, k) do
    root = CSMT.Utils.get_node(tree, root)
    ...
    case result do
      ...
      [key, :MINRS] ->
        [get_with_inclusion_proof(tree, key), nil]

      # Edge Case 2 for non-membership proof
      [:MAXLS, key] ->
        [nil, get_with_inclusion_proof(tree, key)]

      # When a key is bounded by two keys in case of non-membership proof
      [key1, key2] ->
        [get_with_inclusion_proof(tree, key1), get_with_inclusion_proof(tree, key2)]
    end
  end

  # this is called only once, when starting
  defp do_get_with_inclusion_proof(
         tree,
         nil,
         nil,
         root = %CSMT.Nodes.Node{children: [left, right]},
         k
       ) do
    left = CSMT.Utils.get_node(tree, left)
    right = CSMT.Utils.get_node(tree, right)
  ...
  defp do_get_with_inclusion_proof(
         ...
         sibling,
         direction,
         leaf = %CSMT.Nodes.Node{hash: salted_hash, children: [], metadata: value, key: key},
         k
       ) do
    if key == k do
    ...
    cond do
      ...
      l_dist < r_dist ->
        ...
        case {result, direction} do
          # membership proof case
          {[{_, _} | _], _} ->
            [{sibling.hash, rev_dir(direction)} | result]
  ...
  defp min_in_subtree(tree, %CSMT.Nodes.Node{children: [left, _]}) do
    min_in_subtree(tree, CSMT.Utils.get_node(tree, left))
  end

  defp min_in_subtree(_, %CSMT.Nodes.Node{children: [], key: key}) do
    key
  end
  ...
  @spec delete!(CSMT.Types.tree(), binary) :: CSMT.Types.tree()
  def delete!(tree = %CSMT.Trees.Tree{root: root, size: size}, k) do
    root = CSMT.Utils.get_node(tree, root)
  ...
  defp do_delete(tree, root = %CSMT.Nodes.Node{children: [left, right]}, k) do
    left = CSMT.Utils.get_node(tree, left)
    right = CSMT.Utils.get_node(tree, right)
    ...
    if check_for_leaf(left, right, k) do
      if left.key == k do
        ...
        CSMT.Utils.delete_node(tree, left.hash)
        right
      else
        # deletes the target key
        CSMT.Utils.delete_node(tree, right.hash)
        left
      end
    else
      l_dist = distance(k, left.key)
  ...
  defp do_delete(_, leaf = %CSMT.Nodes.Node{children: [], key: key}, k) do
    if key == k do
  ...
  def audit_tree(tree) do
    root = CSMT.Utils.get_node(tree, tree.root)
    do_audit_tree(tree, root, []) |> List.flatten()
  end

  defp do_audit_tree(tree, root = %CSMT.Nodes.Node{children: [left, right]}, acc) do
    left = CSMT.Utils.get_node(tree, left)
    right = CSMT.Utils.get_node(tree, right)

    [do_audit_tree(tree, left, ["L" | acc])] ++ [do_audit_tree(tree, right, ["R" | acc])]
  end

  defp do_audit_tree(_, leaf = %CSMT.Nodes.Node{children: [], metadata: m}, acc) do
  ...
  @spec verify_audit_proof(CSMT.Types.tree(), CSMT.Types.audit_proof()) :: boolean
  ...
  defp do_verify_audit_proof(leaf_hash, [{hash, direction} | t], tree) do
    case direction do
      "L" -> CSMT.Utils.make_hash(tree, hash <> leaf_hash) |> do_verify_audit_proof(t, tree)
      "R" -> CSMT.Utils.make_hash(tree, leaf_hash <> hash) |> do_verify_audit_proof(t, tree)
    end
  end
end
